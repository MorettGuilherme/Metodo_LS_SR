...
# Objetivo do código: cálculo do desempenho do método LS sem restrição para a estimação da amplitude, fase ou pedestal pela validação cruzada K-Fold.
...
1) Instrução para salvar em arquivos os dados estatísticos do desempenho do método LS sem restrição.
...
2) Função para o cálculo do desempenho do método LS sem restrição pelo Erro Médio de Estimação (EME).
...
3) Função para o cálculo do desempenho do método LS sem restrição pelo Erro Médio Quadrático (Mean Squared Error - MSE).
Entrada: número de elementos presentes em cada bloco e lista do erro de estimação para cada bloco do K-Fold.
Saída: média do MSE, variância do MSE e desvio padrão do MSE.

4) Função para o cálculo do desempenho do método LS sem restrição pelo Erro Médio Absoluto (Mean Absolute Error - MAE).
Entrada: número de elementos presentes em cada bloco e lista do erro de estimação para cada bloco do K-Fold.
Saída: média do MAE, variância do MAE e desvio padrão do MAE.

5) Função para o cálculo do desempenho do método LS sem restrição pela Relação Sinal-Ruído (Signal-to-Noise Ratio - SNR).
...
6) Função para o cálculo do desempenho do método LS sem restrição pelo desvio padrão (DP).
Entrada: número de elementos presentes em cada bloco e lista do erro de estimação para cada bloco do K-Fold.
Saída: valor do desvio padrão de cada bloco.

7) Instrução da validação cruzada K-Fold adaptada para o cálculo do desempenho do método LS sem restrição.
...
titulo_programa = colored("Geração de arquivos de saída pela técnica de validação cruzada K-Fold para a estimação da amplitude, fase ou pedestal pelo Métodos dos Mínimos Quadrados Sem Restrição (LS):\n", attrs=["bold"])
...
# Definição da instrução para salvar os dados estatísticos do desempenho do método LS sem restrição em arquivo de saída.
def arquivo_saida_dados_desempenho_LS_SR(parametro, n_ocupacao, n_janelamento_ideal, media_dado_desempenho, var_dado_desempenho, DP_dado_desempenho, mecanismo_desempenho):
    ...
    pasta_saida = f"K_Fold_{parametro}_{mecanismo_desempenho}_Desempenho_LS_SR_OC"
    ...
    arquivo_saida = f"k_fold_{parametro}_{mecanismo_desempenho}_desempenho_LS_SR_J_{n_janelamento_ideal}.txt"
...
### --------- 7) INSTRUÇÃO PARA A VALIDAÇÃO CRUZADA K-FOLD ADAPTADA PARA O CÁLCULO DO DESEMPENHO DO MÉTODO LS SEM RESTRIÇÃO -------------------- ###

# Definição da instrução da técnica de validação cruzada K-Fold para o cálculo do desempenho do método LS sem restrição.
def K_fold_desempenho_LS_SR(parametro, n_ocupacao, n_janelamento_ideal, opcao_avaliacao_desempenho, Matriz_Pulsos_Sinais_Janelado, vetor_parametro_referencia_janelado):
    ...
    for indice_teste in range(0, len(blocos_pulsos_sinais)):
        ...
        # A variável bloco_lista_erro_parametro recebe o valor de retorno da função metodo_LS_SR.
        bloco_vetor_erro_parametro = metodo_LS_SR(n_janelamento_ideal, bloco_treino_pulsos_sinais, bloco_teste_pulsos_sinais, bloco_treino_parametro_referencia, bloco_teste_parametro_referencia)
        
        # Caso a variável opcao_avaliacao_desempenho seja igual a 1.
        if opcao_avaliacao_desempenho == 1:
            
            # A variável bloco_valor_EME recebe o valor de retorno da função EME.
            bloco_valor_EME = EME(quantidade_elementos_bloco, bloco_vetor_erro_parametro)
        ...
        # Caso a variável opcao_avaliacao_desempenho seja igual a 2.
            ...
            bloco_valor_MSE = MSE(quantidade_elementos_bloco, bloco_vetor_erro_parametro)
        ...
        # Caso a variável opcao_avaliacao_desempenho seja igual a 3.
            ...
            bloco_valor_MAE = MAE(quantidade_elementos_bloco, bloco_vetor_erro_parametro)
        ...
        # Caso a variável opcao_avaliacao_desempenho seja igual a 4.
           ...
           bloco_valor_SNR = SNR(bloco_teste_parametro_referencia, bloco_vetor_erro_parametro)
        ...
        # Caso a variável opcao_avaliacao_desempenho seja igual a 5.
           ...
           bloco_valor_DP = DP(quantidade_elementos_bloco, bloco_vetor_erro_parametro)
    ...
    # Salva as informações dos dados estatísticos da análise do desempenho do método LS sem restrição.
    arquivo_saida_dados_desempenho_LS_SR(parametro, n_ocupacao, n_janelamento_ideal, media_desempenho, var_desempenho, DP_desempenho, mecanismo_desempenho)   
...
def principal_desempenho_LS_SR():
    ...
    # A variável n_janelamento_ideal recebe o valor do janelamento ideal do método LS sem restrição.
    # Obs.: essa análise deve ser realizada previamento pela interpretação dos gráficos gerados pelo K-Fold (grafico_k_fold_LS_SR).
    ...
    for n_ocupacao in tqdm(range(ocupacao_inicial, ocupacao_final+1, incremento_ocupacao)):
        ...
        K_fold_desempenho_LS_SR(parametro, n_ocupacao, n_janelamento_ideal, opcao_avaliacao_desempenho, Matriz_Pulsos_Sinais_Janelado, vetor_parametro_referencia_janelado)
