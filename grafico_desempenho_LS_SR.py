...
1) Função para a leitura dos dados do desempenho do método LS sem restrição de todas as ocupações para o janelamento ideal.
Entrada: parâmetro estimado, número do janelamento ideal, opção de avaliação do desempenho.
Saída: matriz com os dados de entrada organizados de acordo com a coluna (número da ocupação, média, variância e desvio padrão do desempenho do método LS sem restrição).

2) Instrução para o plote do gráfico do desempenho do método LS sem restrição ao longo das ocupações para o janelamento ideal.
Entrada: matriz dos dados de desempenho do método LS sem restrição.
Saída: nada.
...
titulo_programa = colored("Plote do gráfico do desempenho (EME, MSE, MAE, SNR ou DP) ao longo das ocupações de acordo com o janelamento ideal para o Método dos Mínimos Quadrados Sem Restrição (LS):\n", attrs=["bold"])
...
def leitura_dados_estatisticos_desempenho_LS_SR(parametro, n_janelamento_ideal, opcao_avaliacao_desempenho):
    ...
    pasta_dados_estatisticos_desempenho = f"K_Fold_{parametro}_{opcao_avaliacao_desempenho}_Desempenho_LS_SR_OC"

    # Nome do arquivo de entrada dos dados estatísticos do desempenho de acordo com o janelamento ideal.
    arquivo_dados_estatisticos_desempenho = f"k_fold_{parametro}_{opcao_avaliacao_desempenho}_desempenho_LS_SR_J_{n_janelamento_ideal}.txt"
...
# Definição da instrução para o plote do gráfico do dado estatístico do desempenho ao longo das ocupações para o janelamento ideal pelo método LS sem restrição.
def grafico_dado_estatistico_desempenho_LS_SR(parametro, opcao_avaliacao_desempenho, Matriz_Dados_Desempenho):
