...
titulo_programa = colored("Plote do gráfico do dado estatístico do erro de estimação da amplitude, fase ou pedestal ao longo das ocupações para um determinado janelamento pelo Métodos Mínimos Quadrados Sem Restrição (LS):\n", attrs=["bold"])
...
def leitura_dados_estatisticos_janelamento_LS_SR(parametro, n_janelamento):
    ...
    pasta_dados_estatisticos_janelamento = f"Dados_Estatisticos_LS_SR_{parametro}_OC"

    # Nome do arquivo de entrada dos dados estatísticos de acordo com o janelamento.
    arquivo_dados_estatisticos_janelamento = f"dados_estatisticos_LS_SR_{parametro}_janelamento_{n_janelamento}.txt"
...
# Definição da instrução para o plote do gráfico do dado estatístico ao longo das ocupações para um determinado janelamento pelo método LS sem restrição.
def grafico_dado_estatistico_janelamento_LS_SR(parametro, dado_estatistico, Matriz_Dados_Estatisticos_Janelamento):
...
def principal_grafico_dado_estatistico_janelamento_LS_SR():
    ...
    Matriz_Dados_Estatisticos_Janelamento = leitura_dados_estatisticos_janelamento_LS_SR(parametro, n_janelamento)
    grafico_dado_estatistico_janelamento_LS_SR(parametro, dado_estatistico, Matriz_Dados_Estatisticos_Janelamento)
